...
const PopoverList = styledTS<{ $selectable?: boolean; $isIndented?: boolean }>(
  ...
  margin-inline-start	: ${(props) => props.$isIndented && `${dimensions.unitSpacing}px`};
  ...
  li {
    ...
    white-space: normal;
    font-size: 13px;
    padding-inline-end: ${(props) => props.$selectable && '30px'};
...
const ToggleIcon = styledTS<{ type?: string }>(styled.div)`
  ...
  ${(props) =>
    ...
    `
  top: 8px;
  inset-inline-start${dimensions.unitSpacing * 0.5}px;
