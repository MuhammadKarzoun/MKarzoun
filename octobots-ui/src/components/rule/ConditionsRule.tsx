...
class ConditionsRule extends React.Component<Props, State> {
  ...
  renderDescription(rule) {
    ...
    switch (rule.kind) {
      ...
      case 'currentPageUrl':
        description =
          'Write your desired page URL, excluding domain name. For example: If you want to place your engagement message on https://office.octobots.ai/pricing - then write /pricing';
        break;
      case 'country':
        description =
  ...
  renderAddRule = () => {
    ...
    return (
      <ModalTrigger title='Add rule' hideHeader trigger={trigger} content={content} />
    );
  };
