...
const colorTypes = (type, value) => {
  switch (type) {
    ...
    case 'secondary':
      if (value === 'content') return colors.colorBlack;
      if (value === 'background') return rgba(colors.updatedBgColor, 1);
...
type Props = {
  ...
  type?: 'primary' | 'danger' | 'warning' | 'success' | 'info' | 'secondary';
  color?: string;
  title?: string;
  iconShow?: boolean;
};
...
class Info extends React.PureComponent<Props> {
  ...
  render() {
    ...
    return (
      <InfoBox style={{ background: '#F2F2F2', borderColor: '#F2F2F2', borderRadius: '10px', fontWeight: '500' }} title={title} type={type} color={color} $iconShow={iconShow}>
        {this.renderIcon()}
