...
const Input = styledTS<{
  ...
  padding: ${dimensions.unitSpacing}px 10px;
  color: ${colors.textPrimary};
  border: 1px solid;
  border-color:${(props) =>
...
const SelectWrapper = styledTS<{ $hasError?: boolean }>(styled.div)`
  overflow: hidden;
  border-bottom: ${(props) =>
    props.$hasError && `1px solid ${colors.colorCoreRed}`};
  width: 100%;
  height: ${textInputHeight};
  position: relative;
...
const inputStyle = styledTS<{ disabled?: boolean; color?: string }>(
  ...
  + span {
    ...
    props.color ? rgba(props.color, 0.7) : colors.colorShadowGray};
      ...
      margin-inline-end: calc(${inputHeight} * 0.25);
      top: 53%;
    ...
    &:after {
      ...
      position: absolute;
      top: 56%;
      inset-inline-start: calc(${inputPadding} + ${inputBorderWidth} + ${inputScale} / 2);
...
const Checkbox = styledTS<{ color?: string }>(styled(inputStyle))`
  + span {
    &:after {
      ...
      top: 53%;
      inset-inline-start: calc(1px + ${inputHeight} / 5);
