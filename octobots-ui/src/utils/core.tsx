...
export const getUserAvatar = (user: IUserDoc, width?: number) => {
  ...
  if (!details || !details.avatar) {
    return "/images/avatar-colored.jpeg";
  }
...
export const sendDesktopNotification = (doc: {
  ...
  const notify = () => {
    ...
    const notification = new Notification(doc.title, {
      body: doc.content,
      icon: "/favicon.png",
      dir: "rtl" // it was ltr @MK
    });

    // notify by sound
    const audio = new Audio("/sound/notify.mp3"); // #TODO change it to a better sound @MK
    audio.play();
...
export const getSubdomain = () => {
  const env = (window as any).octoEnv || {};
...
export const getEnv = () => {
  ...
  if (!VERSION || VERSION !== "saas") {
    ...
    for (const envMap of (window as any).envMaps) {
      envs[envMap.name] = localStorage.getItem(`octobots_env_${envMap.name}`);
    }
    return envs;
  }
...
export const getThemeItem = code => {
  const configs = JSON.parse(
    localStorage.getItem("octobots_theme_configs") || "[]"
  );
