...
import { IntegrationMutationVariables } from '@octobots/ui-inbox/src/settings/integrations/types';
...
class RefreshPermissionForm extends React.PureComponent<Props, CommonTypes> {
  ...
  renderFacebookContent = () => {
    const onClick = () => {
      ...
      const url = `${REACT_APP_API_URL}/pl:facebook/login?kind=facebook`;
  ...
  renderInstagramContent = () => {
    const onClick = () => {
      const { REACT_APP_API_URL } = getEnv();
      const url = `${REACT_APP_API_URL}/pl:instagram/login?kind=instagram`;
