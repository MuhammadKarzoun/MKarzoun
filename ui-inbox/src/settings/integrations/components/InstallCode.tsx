...
import { __, getEnv } from '@octobots/ui/src/utils';
...
const swiftSdkInstall = brandCode => {
  ...
  return `Octobots.setup(octoApiUrl: "${REACT_APP_API_URL}", brandId: "${brandCode}")`;
};

const objcSdkInstall = brandCode => {
  const { REACT_APP_API_URL } = getEnv();
  return `[Octobots setupWithOctoApiUrl:@"${REACT_APP_API_URL}" brandId:@"${brandCode}"];`;
};
...
const dependency = `dependencies {
    implementation 'com.github.octobots:octobots-android-sdk:{latest_release}'
  }`;
...
const startChat = `public void onClick(View view) {
  ...
  config.start(email: "info@octobots.co", phone: "88998899", data: customData)
}`;
...
class InstallCode extends React.PureComponent<Props, State> {
    ...
    if (currentTab === 'ios') {
      return (
        <>
          ...
          <ol>
            <li>
              {__('Add Octobots SDK to your iOS project in Xcode:')}
              <MarkdownWrapper>
                <pre>{iosSDK}</pre>
              </MarkdownWrapper>
            ...
            <li>
              {__(
                'Import OctobotsSDK into AppDelegate.swift then paste the following code into'
              )}{' '}
            ...
            <li>
              {__(
                'Import OctobotsSDK into your UIViewController class and you can start Octobots with following options:'
              )}{' '}
              <br />
              <ol type="a">
                <li>
          ...
          <ol>
            ...
            <li>
              {`#import <OctobotsSDK/OctobotsSDK-Swift.h> into`} AppDelegate.swift
            ...
            <li>
              {`#import <OctobotsSDK/OctobotsSDK-Swift.h>`} {__('into your')}
              UIViewController.m{' '}
              {__('class and you can start with following options:')}{' '}
              <br />
              <ol type="a">
                <li>
                  <b>{__('Without user data')}</b>
                  <MarkdownWrapper>
                    <pre>[Octobots startWithData:nil];</pre>
                  </MarkdownWrapper>
                </li>
                <li>
                  <b>{__('With user data')}</b>
      ...
      return (
        <ol>
          ...
          <li>
            ...
            <br />
            <b>* apiHost</b> - {__('octobots-widgets-api server url')} <br />
            <b>* subsHost</b> - {__('octobots-api subscription url')} <br />
            <b>* uploadUrl</b> - {__('octobots-api server url')}
            <MarkdownWrapper>
  ...
  renderContents() {
    ...
    switch (currentTab) {
      case 'basic':
        description = __(
          'Paste the following code before the body tag on every page you want octobots widget to appear'
        );
        script = basicCode;
        action = copied;
        break;
      case 'single':
        description = __(
          'For web apps built with asynchronous JavaScript. Paste the code below in main layout you want octobots chat to appear'
        );
      ...
      case 'googletag':
        description = __(
          'To connect Google Tag Manager to octobots, you must have an active Google Tag Manager account with a published container'
        );
        extraContent = true;
        script = basicCode;
