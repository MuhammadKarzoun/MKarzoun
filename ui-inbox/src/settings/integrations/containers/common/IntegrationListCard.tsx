...
import IntegrationListCard from '../../components/common/IntegrationListCard';
...
const IntegrationListContainer = (props: FinalProps) => {
  ...
  const filteredIntegrations = integrations
    .filter(integration => props.rows.some(row => row.kind === integration.kind))
    .map(integration => {
      const matchingRow = props.rows.find(row => row.kind === integration.kind);
      return {
        ...integration,
        logo: matchingRow?.logo
      };
    });
  ...
  return <IntegrationListCard {...updatedProps} />;
};
