...
// import TextDivider from '@octobots/ui/src/components/TextDivider';
...
const SimpleMessage: React.FC<Props> = React.memo(({
  ...
  // const handleToggleDropdown = () => setHiddenDropdown(!hiddenDropdown);
          ...
          toggleComponent={
            <OptionsContainer $staff={isStaff}>
              <div className="options-logo">
                <i className="icon-downarrow-2"></i>
              </div>
            </OptionsContainer>
          }
          unmount={false}
        ...
        >
          ...
          <MenuItem>
            <a className='dropdown-item' onClick={() => {
              setReplyId(message); // Set the reply ID
              console.log("Replying to message ID:", message._id); // Debugging
            }}>
              <i className="icon-reply"></i>
              {__('Reply to message')}
            </a>
